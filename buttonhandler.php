...
if($buttId=='gen_scdp')
{
	...
	buttonHandler_gen_scdp($params);
}
if($buttId=='gen_scdp_nopaa')
{
	//  for login page users table can be turned off
	if( $table != GLOBAL_PAGES )
	{
	...
	buttonHandler_gen_scdp_nopaa($params);
}
...
function buttonHandler_gen_scdp($params)
{
	global $strTableName;
	$result = array();
...
//$result["txt"] = $params["txt"]." world!";
$record = $button->getCurrentRecord();
$result["PAA_ID_FK"]=$record["PAA_ID_FK"]."&PAAU_ID=".$record["PAAU_ID"];
$resulta["PAAU_ID"]=$record["PAAU_ID"];
;
	RunnerContext::pop();
	echo my_json_encode($result);
	$button->deleteTempFiles();
}
function buttonHandler_gen_scdp_nopaa($params)
{
	global $strTableName;
	$result = array();
	...
	// Put your code here.
$record = $button->getCurrentRecord();
$result["PAA_ID_FK"]=$record["PAA_ID_FK"];
;
	RunnerContext::pop();
	echo my_json_encode($result);
	$button->deleteTempFiles();
