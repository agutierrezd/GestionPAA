...
mod(CodeMirror);})(function(CodeMirror){"use strict";CodeMirror.defineMode("python",function(conf,parserConf){var ERRORCLASS='error';function wordRegexp(words){return new RegExp("^(("+words.join(")|(")+"))\\b");}
...
for(var i=0;i<state.scopes.length;++i){if(state.scopes[i].type==='py'){indentUnit=state.scopes[i].offset+conf.indentUnit;break;}}}else if(stream.match(/\s*($|#)/,false)){indentUnit=stream.indentation()+hangingIndent;}else{indentUnit=stream.column()+stream.current().length;}
