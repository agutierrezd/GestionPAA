...
mod(CodeMirror);})(function(CodeMirror){"use strict";CodeMirror.defineMode("tiddlywiki",function(){var textwords={};var keywords=function(){function kw(type){return{type:type,style:"macro"};}
...
function jsTokenBase(stream,state){var sol=stream.sol(),ch;state.block=false;ch=stream.peek();if(sol&&/[<\/\*{}\-]/.test(ch)){if(stream.match(reCodeBlockStart)){state.block=true;return chain(stream,state,twTokenCode);}
...
ch=stream.next();if(sol&&/[\/\*!#;:>|]/.test(ch)){if(ch=="!"){stream.skipToEnd();return ret("header","header");}
if(ch=="*"){stream.eatWhile('*');return ret("list","comment");}
if(ch=="#"){stream.eatWhile('#');return ret("list","comment");}
if(ch==";"){stream.eatWhile(';');return ret("list","comment");}
if(ch==":"){stream.eatWhile(':');return ret("list","comment");}
...
if(/[hf]/i.test(ch)){if(/[ti]/i.test(stream.peek())&&stream.match(/\b(ttps?|tp|ile):\/\/[\-A-Z0-9+&@#\/%?=~_|$!:,.;]*[A-Z0-9+&@#\/%=~_|$]/i)){return ret("link","link");}}
if(ch=='"'){return ret('string','string');}
if(ch=='~'){return ret('text','brace');}
if(/[\[\]]/.test(ch)){if(stream.peek()==ch){stream.next();return ret('brace','brace');}}
if(ch=="@"){stream.eatWhile(isSpaceName);return ret("link","link");}
...
if(ch=="-"){if(stream.eat("-")){if(stream.peek()!=' ')
...
function twTokenCode(stream,state){var ch,sb=state.block;if(sb&&stream.current()){return ret("code","comment");}
if(!sb&&stream.match(reUntilCodeStop)){state.tokenize=jsTokenBase;return ret("code","comment");}
if(sb&&stream.sol()&&stream.match(reCodeBlockStop)){state.tokenize=jsTokenBase;return ret("code","comment");}
ch=stream.next();return(sb)?ret("code","comment"):ret("code","comment");}
...
function twTokenStrike(stream,state){var maybeEnd=false,ch;while(ch=stream.next()){if(ch=="-"&&maybeEnd){state.tokenize=jsTokenBase;break;}
maybeEnd=(ch=="-");}
return ret("text","strikethrough");}
function twTokenMacro(stream,state){var ch,word,known;if(stream.current()=='<<'){return ret('brace','macro');}
...
return{startState:function(){return{tokenize:jsTokenBase,indented:0,level:0};},token:function(stream,state){if(stream.eatSpace())return null;var style=state.tokenize(stream,state);return style;},electricChars:""};});CodeMirror.defineMIME("text/x-tiddlywiki","tiddlywiki");});